...
local gui_wallet_step(image, wine=false) = {
    ...
    commands: [
        ...
        'cp -v build/bin/beldexd' + (if wine then '.exe' else '') + ' loki-electron-gui-wallet/bin',
        'cp -v build/bin/beldex-wallet-rpc' + (if wine then '.exe' else '') + ' loki-electron-gui-wallet/bin',
...
local gui_wallet_step_darwin = {
    ...
    commands: [
        'git clone https://github.com/loki-project/loki-electron-gui-wallet.git',
        'cp -v build/bin/{beldexd,beldex-wallet-rpc} loki-electron-gui-wallet/bin',
        'cd loki-electron-gui-wallet',
        'sed -i -e \'s/^\\\\( *"version": ".*\\\\)",/\\\\1-${DRONE_COMMIT_SHA:0:8}",/\' package.json',
        'npm install',
        'npm run build',
